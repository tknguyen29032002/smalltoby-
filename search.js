...
(function (root) {
  ...
  function bidiStep(grid, cur, side, name) {
    ...
    return {
      ...
      // A cell can sit in both queues at once, so the drawn list is deduped;
      // frontierForward and frontierBackward keep the per-side truth.
      frontierCells: uniq(fwd.concat(back)),
      frontierForward: fwd,
  ...
  // Playback holds one entry per step and animates at most ~180 a second, so a
  // strategy that re-walks the map has to stop recording somewhere or a single
  // run becomes a ten-minute animation. The counters stay honest past this
  // point, and the trace says `truncatedSteps: true`.
  var MAX_TRACE_STEPS = 4500;
  ...
  function iterativeDeepeningSearch(grid, params) {
    ...
    return finishTrace(grid, {
      strategy: 'iddfs',
      params: params,
      revisits: true,        // every pass re-walks the shallow cells
      steps: steps,
      path: found ? path : [],
      expansions: expansions,
      peakFrontier: peakFrontier,
  ...
  function bellmanFordSearch(grid, params) {
    ...
    return finishTrace(grid, {
      strategy: 'bellman',
      params: params,
      revisits: true,        // a cell is relaxed again whenever it improves
      steps: steps,
      path: path,
      expansions: expansions,
      peakFrontier: peakFrontier,
  ...
  function wallFollowerSearch(grid, params) {
    ...
    return finishTrace(grid, {
      strategy: 'wall',
      params: params,
      revisits: true,        // no visited set at all, so it walks in circles
      steps: steps,
      path: found ? path : [],
      expansions: expansions,
      peakFrontier: 1,
   ...
   * The page loads plain script tags, so every name in here would otherwise be
   * a global that some other file can collide with - game.js has its own
   * runSearch, for one. Everything lives in this closure instead, and only the
   * engine namespace plus the three names the page already used come out. */
  var API = {
  ...
  if (root) {
    root.PathfinderEngine = API;
    root.search = search;
    root.parseGrid = parseGrid;
