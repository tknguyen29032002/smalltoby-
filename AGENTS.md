...
Factory Heist (`index.html`) is data plus one simulator: `campaign.js` and `shop.js` hold encounters, economy and shop and bind to `maps.js` by map id; `heist.js` is the only place a rule lives, and `heist-ui.js` adds none.
Par is measured, not guessed: after touching `campaign.js`, `shop.js` or `maps.js`, run `node tools/verify-campaign.js` (`--write` re-measures par and start charge); after touching those or `heist.js`, also run `node tools/verify-heist.js`, which must win all fifteen floors with the thieves on. The design both enforce is `docs/design/CAMPAIGN.md`.

`npm test` cannot see the page. After touching a page, run its browser walkthrough over both `file://` and `http://` from a fresh profile - `tests/browser/heist-walkthrough.js` for `index.html`/`heist-*.js`, `tests/browser/walkthrough.js` for `training.html`/`game.js`/`render.js`; their headers have the commands. The two runs must report the same digest and no problems, and the heist walkthrough's per-floor rows must equal verify-heist's table.
